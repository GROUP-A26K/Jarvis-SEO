...
function getWeekDates(weekStr) {
  ...
  if (weekStr) {
    if (!/^\d{4}-W\d{1,2}$/.test(weekStr)) {
      console.error(`Format invalide: "${weekStr}". Attendu: YYYY-WNN`);
      process.exit(1);
    }
  ...
  return {
    ...
    prevMonday: new Date(monday.getTime() - 7 * 86400000),
    prevSunday: new Date(monday.getTime() - 86400000),
    weekStr: weekStr || getISOWeek(monday),
...
async function checkArticlePositions(semrushKey, articles, today) {
  ...
  for (const art of articles) {
    ...
    let checkpoint = null,
      field = null;
    if (art.j30_date && todayStr >= art.j30_date.split('T')[0] && !art.position_j30) {
      checkpoint = 'J+30';
      field = 'position_j30';
    } else if (art.j60_date && todayStr >= art.j60_date.split('T')[0] && !art.position_j60) {
      checkpoint = 'J+60';
      field = 'position_j60';
    } else if (art.j90_date && todayStr >= art.j90_date.split('T')[0] && !art.position_j90) {
      checkpoint = 'J+90';
      field = 'position_j90';
    }
    ...
    try {
      if (!cache[art.site]) {
        const resp = await rateLimitedSemrushGet(
          `https://api.semrush.com/?type=domain_organic&key=${semrushKey}&domain=${art.site}&database=ch&export_columns=Ph,Po&display_limit=200`,
        );
        trackUnits('domain_organic', 10);
        const map = {};
        for (const line of resp.trim().split('\n').slice(1)) {
          const p = line.split(';');
          const kw = (p[0] || '').trim().toLowerCase();
          if (kw) map[kw] = parseInt(p[1], 10) || null;
        }
        cache[art.site] = map;
      }
      ...
      if (position !== null) {
        ...
        results.push({
          ...
          checkpoint,
          position,
          status: position <= 10 ? 'ok' : position <= 20 ? 'mid' : 'low',
          slug: art.slug,
        });
        console.log(
          `  ${position <= 10 ? '+' : position <= 20 ? '~' : '-'} ${art.keyword}: pos ${position} (${checkpoint})`,
        );
      }
    } catch (e) {
...
function analyzeCTR(trackedArticles, gscDataBySite) {
  ...
  for (const art of trackedArticles) {
    ...
    if (ratio < 0.5) {
      ...
      console.log(
        `  ! ${art.keyword} (pos ${pos}): CTR ${(actualCtr * 100).toFixed(1)}% vs attendu ${(expectedCtr * 100).toFixed(1)}% -> ameliorer title/meta`,
      );
    }
  }
  return alerts;
...
function detectContentDecay(trackedArticles, semrushPositions) {
  ...
  for (const art of trackedArticles) {
    ...
    const bestHistorical = Math.min(
      art.position_j30 || 999,
      art.position_j60 || 999,
      art.position_j90 || 999,
    );
    ...
    if (currentPos && bestHistorical < 999 && currentPos > bestHistorical + 10) {
      ...
      console.log(
        `  ! ${art.keyword}: pos ${bestHistorical} -> ${currentPos} (-${currentPos - bestHistorical}, ${Math.round(monthsOld)} mois)`,
      );
    }
  }
  return decayed;
  ...
  for (const art of toCheck) {
    try {
      const resp = await callClaudeWithRetry(
        apiKey,
        'Analyste GEO. JSON: { "visibility": "cited|partial|absent", "confidence": 0-10 }',
        `"${art.keyword}" sur ${art.site}: cite par Google AI Overview / Perplexity ?`,
        200,
      );
      const r = JSON.parse(
      ...
      console.log(
        `  ${vis === 'cited' ? '+' : vis === 'partial' ? '~' : '-'} ${art.keyword}: ${vis}`,
      );
    } catch (e) {
      logger.debug('catch silencieux', { error: e.message });
    }
...
async function getGoogleToken(oauth) {
  const r = await httpRequest('https://oauth2.googleapis.com/token', {
    ...
    body: new URLSearchParams({
      ...
      refresh_token: oauth.web.refresh_token,
...
async function fetchGSC(token, site, start, end) {
  try {
    ...
    logger.warn(`GSC ${site}: ${e.message}`);
    return [];
  }
}
...
function aggregateGSC(rows) {
  ...
  const pages = Object.entries(pm)
    .map(([url, d]) => ({
      url,
      title: url.split('/').pop() || url,
      impressions: d.imp,
      clicks: d.clk,
      ctr: d.imp > 0 ? d.clk / d.imp : 0,
      position: d.posW > 0 ? Math.round((d.posS / d.posW) * 10) / 10 : 0,
    }))
  ...
  return {
    impressions: imp,
    clicks: clk,
    ctr: Math.round(imp > 0 ? (clk / imp) * 10000 : 0) / 100,
    position: Math.round(imp > 0 ? (posW / imp) * 10 : 0) / 10,
    pages,
  };
}
...
async function getSemrushPositions(apiKey, domain) {
  try {
    const resp = await rateLimitedSemrushGet(
      `https://api.semrush.com/?type=domain_organic&key=${apiKey}&domain=${domain}&database=ch&export_columns=Ph,Po,Nq&display_limit=50`,
    );
    ...
    logger.debug('Semrush positions indisponibles', { error: e.message });
    return [];
  }
}
...
function generateHTML(data) {
  ...
  for (const s of Object.values(sites)) {
    tI += s.current.impressions;
    tC += s.current.clicks;
    tPI += s.previous.impressions;
    tPC += s.previous.clicks;
    tPos += s.current.position;
    tPPos += s.previous.position;
    sc++;
  }
  const avgP = sc > 0 ? tPos / sc : 0,
    avgPP = sc > 0 ? tPPos / sc : 0;
  const ctr = tI > 0 ? (tC / tI) * 100 : 0,
    pCtr = tPI > 0 ? (tPC / tPI) * 100 : 0;
  ...
  for (const [domain, sd] of Object.entries(sites)) {
    ...
    let rows = pages
      ...
      .map(
        (p) =>
          `<tr><td class="page-title">${esc(p.title)}</td><td>${p.position}</td><td>${(p.ctr * 100).toFixed(1)}%</td><td>${dot(p.position)}</td></tr>`,
      )
      .join('');
  ...
  if (gapAnalysis) {
    const allOpps = [];
    for (const [d, sg] of Object.entries(gapAnalysis.sites || {})) {
      for (const o of (sg.opportunities || []).slice(0, 3)) allOpps.push({ ...o, domain: d });
    }
    const top3 = allOpps.sort((a, b) => b.score - a.score).slice(0, 3);
    if (top3.length)
      gapSection = `<div class="gap-section"><h3>Top 3 Opportunites</h3><table><thead><tr><th>Mot-cle</th><th>Site</th><th>Vol</th><th>Score</th></tr></thead><tbody>${top3.map((o) => `<tr><td><strong>${esc(o.keyword)}</strong></td><td>${esc(o.domain)}</td><td>${o.volume}</td><td>${o.score}</td></tr>`).join('')}</tbody></table></div>`;
  }

  const eI = tPI > 0 ? ((tI - tPI) / tPI) * 100 : 0,
    eC = tPC > 0 ? ((tC - tPC) / tPC) * 100 : 0;
  const eCtr = pCtr > 0 ? ((ctr - pCtr) / pCtr) * 100 : 0,
    eP = avgPP > 0 ? ((avgPP - avgP) / avgPP) * 100 : 0;
...
async function main() {
  ...
  let weekParam = null;
  if (wIdx !== -1) {
    weekParam = args[wIdx + 1];
    if (!weekParam || weekParam.startsWith('--')) {
      console.error('--week requiert une valeur');
      process.exit(1);
    }
  }
  const dates = getWeekDates(weekParam);
  console.log(
    `  Semaine: ${dates.weekStr} | ${formatDate(dates.monday)} -> ${formatDate(dates.sunday)}`,
  );
  ...
  let token = null;
  try {
    token = await getGoogleToken(googleOAuth);
  ...
  console.log('\n> Generation rapport');
  const reportData = {
    weekStr: dates.weekStr,
    year: dates.year,
    period: `${formatDate(dates.monday)} -> ${formatDate(dates.sunday)}`,
    sites: sitesData,
    gapAnalysis: gap,
    articleTracking,
    ctrAlerts,
