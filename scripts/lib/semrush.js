...
const { SEMRUSH_SESSION_LIMIT, SEMRUSH_INTERVAL_MS, RETRY, TIMEOUTS } = require('./constants');
...
function semrushSessionGuard(estimatedUnits) {
  if (_semrushSession.tripped) {
    throw new Error(
      `[SEMRUSH GUARD] Disjoncteur de session actif — limite ${SEMRUSH_SESSION_LIMIT} unités atteinte (${_semrushSession.consumed} consommées). Arrêt de sécurité.`,
    );
  }
  ...
  if (_semrushSession.consumed + est > SEMRUSH_SESSION_LIMIT) {
    ...
    throw new Error(
      `[SEMRUSH GUARD] Limite de session ${SEMRUSH_SESSION_LIMIT} unités dépassée (${_semrushSession.consumed} + ${est} estimées). Arrêt de sécurité.`,
    );
  }
}
...
function semrushSessionRecord(units) {
  ...
  if (pct >= 80 && !_semrushSession._warned80) {
    ...
    logger.warn(
      `[SEMRUSH GUARD] Session: ${_semrushSession.consumed}/${SEMRUSH_SESSION_LIMIT} unités (${pct}%) — ${SEMRUSH_SESSION_LIMIT - _semrushSession.consumed} restantes`,
    );
  }
  if (_semrushSession.consumed >= SEMRUSH_SESSION_LIMIT) {
    _semrushSession.tripped = true;
    logger.warn(
      `[SEMRUSH GUARD] Limite de session atteinte — plus aucun appel Semrush autorisé pour ce process`,
    );
  }
}
...
function rateLimitedSemrushGet(url) {
  ...
  if (!circuitBreakers.semrush.canExecute()) {
    return Promise.reject(
      new Error('Semrush circuit breaker OUVERT — service temporairement indisponible'),
    );
  }
    ...
    .get(url, (res) => {
      ...
      res.on('end', () => {
        ...
        if (res.statusCode !== 200) {
          circuitBreakers.semrush.recordFailure();
          reject(
            new Error(`Semrush ${res.statusCode}: ${sanitizeErrorMessage(data.slice(0, 150))}`),
          );
          return;
        }
      ...
      res.on('error', (e) => {
        circuitBreakers.semrush.recordFailure();
        reject(new Error(`Semrush stream: ${sanitizeErrorMessage(e.message)}`));
      });
    })
    .on('error', (e) => {
      circuitBreakers.semrush.recordFailure();
      reject(new Error(`Semrush request: ${sanitizeErrorMessage(e.message)}`));
    });
...
function rateLimitedSemrushRequest(params) {
  ...
  try {
    semrushSessionGuard(estimatedUnits);
  } catch (e) {
    return Promise.reject(e);
  }
...
function validateSemrushData(domain, rowCount) {
  ...
  if (suspicious) {
    logger.warn(
      `ALERTE: ${domain} retourne ${rowCount} kws (precedent: ${prev}). Possible erreur API Semrush.`,
    );
  }
