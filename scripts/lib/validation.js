...
function validateArticleInput(art) {
  ...
  if (art.keyword && /[;|&`$(){}\\]/.test(art.keyword))
    errors.push(`keyword contient des caracteres interdits: "${art.keyword}"`);
  if (art.keyword && art.keyword.length > 100) errors.push('keyword trop long (max 100)');
  if (art.persona && !VALID_PERSONAS.includes(art.persona))
    errors.push(`persona invalide: "${art.persona}"`);
  return errors;
}
