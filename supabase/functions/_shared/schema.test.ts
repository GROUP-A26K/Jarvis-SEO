...
Deno.test("property_id rejects 7 digits", () => {
  assertThrows(
    () => SiteMetricsSchema.parse({ ...baseSite, property_id: "1234567" }),
    z.ZodError,
  );
});
...
Deno.test("slug rejects underscore 'fg_test'", () => {
  assertThrows(
    () => SiteMetricsSchema.parse({ ...baseSiteForSlug, slug: "fg_test" }),
    z.ZodError,
  );
});
...
Deno.test("slug rejects single char 'f' (min 2)", () => {
  assertThrows(
    () => SiteMetricsSchema.parse({ ...baseSiteForSlug, slug: "f" }),
    z.ZodError,
  );
});
