...
test('escapes &', () => {
  assertEqual(shared.esc('A & B'), 'A &amp; B');
});
test('escapes <>', () => {
  assertEqual(shared.esc('<script>'), '&lt;script&gt;');
});
test('escapes quotes', () => {
...
test('returns default for corrupted JSON', () => {
  ...
  try {
    fs.readdirSync(os.tmpdir())
      .filter((f) => f.startsWith(`test-corrupt-${Date.now()}`))
...
test('withLockedJSON mutates and saves', () => {
  ...
  shared.withLockedJSON(tmp, { count: 0 }, (data) => {
    data.count++;
  });
  const result = shared.readJSONSafe(tmp, {});
...
test('invalid persona fails', () => {
  const errors = shared.validateArticleInput({
    site: 'medcourtage.ch',
    keyword: 'test',
    persona: 'Fake Person',
  });
    ...
    sections: [
      {
        heading: 'Section 1',
        content:
          'Content with enough words to pass the minimum check for validation testing purposes in this test suite',
      },
    ],
...
test('valid prompt passes', () => {
  const prompt =
    'A medical office desk captured with Canon R5, 50mm f/2.8, warm afternoon light through blinds, slight dust motes visible, pen resting on insurance documents, coffee ring stain on paper, shallow depth of field with clinical white walls';
...
test('roundtrip: error result is readable with correct code', () => {
  ...
  shared.finalizeError(r, {
    code: shared.ERROR_CODES.DUPLICATE_KEYWORD,
    message: 'dup test',
    stage: 'init',
    retryable: false,
  });
...
test('handlers/task-handlers.js passes --output-json to pipeline', () => {
  const src = fs.readFileSync(
    path.join(__dirname, '..', 'scripts', 'handlers', 'task-handlers.js'),
    'utf-8',
  );
...
test('workflow-daily.js no longer defines its own runArticle', () => {
  ...
  assert(
    !src.includes('function runArticle('),
    'workflow-daily should not define runArticle locally',
  );
});
test('workflow-single-task.js no longer defines its own runArticle', () => {
  const src = fs.readFileSync(
    path.join(__dirname, '..', 'scripts', 'workflow-single-task.js'),
    'utf-8',
  );
  assert(
    !src.includes('function runArticle('),
    'workflow-single-task should not define runArticle locally',
  );
});
...
test('workflow-single-task no longer parses stdout with match regex', () => {
  ...
  assert(
    !src.includes('stdout.match('),
    'workflow-single-task should not parse stdout with .match()',
  );
});
...
test('workflow-daily.js imports from handlers/task-handlers', () => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'scripts', 'workflow-daily.js'), 'utf-8');
  assert(
    src.includes("require('./handlers/task-handlers')"),
    'workflow-daily should import from ./handlers/task-handlers',
  );
});
test('workflow-single-task.js imports from handlers/task-handlers', () => {
  ...
  assert(
    src.includes("require('./handlers/task-handlers')"),
    'workflow-single-task should import from ./handlers/task-handlers',
  );
});
test('workflow-daily.js no longer defines sendPublicationNotification', () => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'scripts', 'workflow-daily.js'), 'utf-8');
  assert(
    !src.includes('async function sendPublicationNotification('),
    'workflow-daily should not define sendPublicationNotification locally',
  );
});
test('workflow-single-task.js no longer defines sendPublicationNotification', () => {
  ...
  assert(
    !src.includes('async function sendPublicationNotification('),
    'workflow-single-task should not define sendPublicationNotification locally',
  );
});
...
test('workflow-single-task.js delegates regenerate_exhibit to handler', () => {
  ...
  assert(
    src.includes('handleRegenerateExhibit(task,'),
    'workflow-single-task should call handleRegenerateExhibit',
  );
});
test('workflow-single-task.js no longer has inline planExhibits require', () => {
  ...
  assert(
    !src.includes("require('./seo-exhibits')"),
    'workflow-single-task should not inline-require ./seo-exhibits anymore',
  );
});
...
test('workflow-daily.js delegates scheduled publications to handler', () => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'scripts', 'workflow-daily.js'), 'utf-8');
  assert(
    src.includes('handleScheduledPublication(pub,'),
    'workflow-daily should call handleScheduledPublication',
  );
});
test('workflow-daily.js no longer calls runArticle directly in pub loop', () => {
  ...
  assert(
    !pubLoopBody.includes('runArticle('),
    'pub loop should delegate to handler (no direct runArticle call)',
  );
});
...
test('workflow-single-task.js delegates publish_draft to handler', () => {
  ...
  assert(
    src.includes('handlePublishDraft(task,'),
    'workflow-single-task should call handlePublishDraft',
  );
});
test('workflow-daily.js no longer calls publishToSanity directly', () => {
  ...
  assert(
    !src.includes('await publishToSanity('),
    'workflow-daily should not call publishToSanity directly',
  );
});
test('workflow-single-task.js no longer calls publishToSanity directly in publish_draft', () => {
  ...
  assert(
    !src.includes('await publishToSanity('),
    'workflow-single-task should not call publishToSanity directly',
  );
});
...
test('workflow-daily.js delegates generate_article to handler', () => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'scripts', 'workflow-daily.js'), 'utf-8');
  assert(
    src.includes('handleGenerateArticle(task,'),
    'workflow-daily should call handleGenerateArticle',
  );
});
test('handlers/task-handlers.js handlePublishDraft builds content_url with /blog/ prefix and strips language prefix', () => {
  ...
  assert(
    ...
    'handlePublishDraft should build contentUrl with /blog/ and strip fr-/en- prefix',
  );
});
test('handlers/task-handlers.js no longer builds bare https://site/slug URLs', () => {
  ...
  assert(
    !src.includes('`https://${site}/${article.slug}`'),
    'task-handlers should not use the bare site/slug formula anymore — must include /blog/ and strip language prefix',
  );
});
test('workflow-daily.js imports uploadExhibitToStorage from calendar-connector', () => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'scripts', 'workflow-daily.js'), 'utf-8');
  assert(
    src.includes('uploadExhibitToStorage'),
    'workflow-daily should import uploadExhibitToStorage from calendar-connector (parity with single-task)',
  );
});
test('workflow-daily.js passes uploadExhibitsToStorage: true and the adapter in ctx', () => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'scripts', 'workflow-daily.js'), 'utf-8');
  assert(
    src.includes('uploadExhibitsToStorage: true'),
    'workflow-daily should enable uploadExhibitsToStorage (parity with single-task)',
  );
  assert(
    !src.includes('uploadExhibitsToStorage: false'),
    'workflow-daily should not pass uploadExhibitsToStorage: false anymore',
  );
  assert(
    src.match(/fetchSiteAdmins,\s*uploadExhibitToStorage,/),
    'workflow-daily should include uploadExhibitToStorage in the handleGenerateArticle ctx',
  );
});
test('workflow-single-task.js delegates generate_article to handler', () => {
  ...
  assert(
    src.includes('handleGenerateArticle(task,'),
    'workflow-single-task should call handleGenerateArticle',
  );
});
test('workflow-daily.js no longer imports sanitize/validateArticleInput', () => {
  ...
  assert(
    !src.includes('validateArticleInput'),
    'workflow-daily should not need validateArticleInput anymore',
  );
});
test('workflow-single-task.js no longer imports path/fs/SCRIPTS_DIR at top', () => {
  ...
  assert(
    !src.includes('const SCRIPTS_DIR ='),
    'workflow-single-task should not define SCRIPTS_DIR anymore',
  );
});
...
test('exports exactly the 6 expected symbols', () => {
  ...
  assertEqual(
    keys,
    'handleGenerateArticle,handlePublishDraft,handleRegenerateExhibit,handleScheduledPublication,runArticle,sendPublicationNotification',
  );
});
...
test('sendPublicationNotification is async, runArticle is sync', () => {
  ...
  assertEqual(
    h.runArticle.constructor.name,
    'Function',
    'runArticle uses execFileSync, should be sync',
  );
});
test('workflow-daily.js final line count <= 200 (dispatcher size sanity)', () => {
  ...
  assert(
    lineCount <= 200,
    `workflow-daily.js should be <= 200 lines post-PR-0.4, got ${lineCount}`,
  );
});
test('workflow-single-task.js final line count <= 200 (dispatcher size sanity)', () => {
  ...
  assert(
    lineCount <= 200,
    `workflow-single-task.js should be <= 200 lines post-PR-0.4, got ${lineCount}`,
  );
});
test('scripts/handlers/README.md documents the layer', () => {
  const readme = fs.readFileSync(
    path.join(__dirname, '..', 'scripts', 'handlers', 'README.md'),
    'utf-8',
  );
...
test('seo-publish-article.js catch Sanity FR propagates errors (throw err)', () => {
  const src = fs.readFileSync(
    path.join(__dirname, '..', 'scripts', 'seo-publish-article.js'),
    'utf-8',
  );
  assert(
    normalizeSource(src).includes(
      normalizeSource(
        '} catch (err) { console.error(`  ! Sanity FR: ${err.message}`); err.errorCode = ERROR_CODES.SANITY_PUBLISH_FAILED; throw err; }',
      ),
    ),
    'seo-publish-article.js FR catch must tag err.errorCode and re-throw so the pipeline marks the task as failed with the correct SANITY_PUBLISH_FAILED code',
  );
});
test('seo-publish-article.js catch Sanity EN propagates errors (throw err)', () => {
  ...
  assert(
    ...
    'seo-publish-article.js EN catch must tag err.errorCode and re-throw so a partial publication is marked as failed with the correct SANITY_PUBLISH_FAILED code',
  );
});
test('seo-publish-article.js no longer has the silent Sanity FR catch', () => {
  ...
  assert(
    !src.includes('} catch (err) { console.error(`  ! Sanity FR: ${err.message}`); }'),
    'seo-publish-article.js must not swallow Sanity FR errors silently — the silent catch form should be replaced with one that re-throws',
  );
});
test('seo-publish-article.js no longer has the silent Sanity EN catch', () => {
  ...
  assert(
    !src.includes('} catch (err) { console.error(`  ! Sanity EN: ${err.message}`); }'),
    'seo-publish-article.js must not swallow Sanity EN errors silently — the silent catch form should be replaced with one that re-throws',
  );
});
...
test('seo-publish-article.js tags FR catch with ERROR_CODES.SANITY_PUBLISH_FAILED', () => {
  ...
  assert(
    src.includes('! Sanity FR:') &&
      src.includes('err.errorCode = ERROR_CODES.SANITY_PUBLISH_FAILED'),
    'FR catch must tag err.errorCode = ERROR_CODES.SANITY_PUBLISH_FAILED before throw so routing does not depend on err.message',
  );
});
test('seo-publish-article.js tags EN catch with ERROR_CODES.SANITY_PUBLISH_FAILED', () => {
  ...
  assert(
    src.includes('! Sanity EN:') &&
      src.includes('err.errorCode = ERROR_CODES.SANITY_PUBLISH_FAILED'),
    'EN catch must tag err.errorCode = ERROR_CODES.SANITY_PUBLISH_FAILED before throw so routing does not depend on err.message',
  );
});
test('seo-publish-article.js main().catch() prefers err.errorCode over err.message regex', () => {
  ...
  assert(
    normalizeSource(src).includes(normalizeSource('const code = err.errorCode')),
    'main().catch() must read err.errorCode first before falling back to message regex',
  );
  assert(
    !src.includes('/sanity/i.test(err.message)'),
    'main().catch() must not use /sanity/i regex anymore — use err.errorCode tag instead',
  );
});
